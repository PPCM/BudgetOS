...
export class CreditCard {
  ...
  static async create(userId, data) {
    ...
    await knex('credit_cards').insert({
      ...
      has_dedicated_account: data.hasDedicatedAccount ? 1 : 0,
      auto_generate_debit: data.autoGenerateDebit !== false ? 1 : 0,
  ...
  static async findById(id, userId) {
    const card = await knex('credit_cards as cc')
      ...
      .where({ 'cc.id': id, 'cc.user_id': userId, 'cc.is_active': 1 })
      .first();
  ...
  static async findByUser(userId, options = {}) {
    ...
    if (isActive !== undefined) {
      query = query.where('cc.is_active', isActive ? 1 : 0);
    }
